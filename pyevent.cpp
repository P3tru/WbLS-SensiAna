...
void Particle::setProperties(string name, string gen_process, string end_process, TVector3 *steps, double *times, double *energies, int nSteps) {
    this->name = name;
...
void extractTree(const char *file, int event_num) {
    ...
    for (int j = 0; j < nTracks; j++) {
        ...
        double *times = new double[nSteps];
        double *energies = new double[nSteps];
        for (int k = 0; k < nSteps; k++) {
            steps[k] = track->GetMCTrackStep(k)->GetEndpoint();
            times[k] = track->GetMCTrackStep(k)->GetGlobalTime();
            energies[k] = track->GetMCTrackStep(k)->GetKE();
        }

        cur->setProperties(track->GetParticleName(),first->GetProcess(),last->GetProcess(),steps,times,energies,nSteps);
    }
